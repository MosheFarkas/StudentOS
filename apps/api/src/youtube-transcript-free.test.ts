...
describe('relay parameter mapping', () => {
  /**
   * The bug this pins. FetchParams is FLAT -- {url, method, body, headers,
   * userAgent} -- not a fetch-style {url, init}. Reading params.init gave
   * undefined, so every request went out as a bare GET with no body, and
   * YouTube answered "this video is no longer available".
   *
   * That is the worst shape of wrong answer: it looks like a fact about the
   * video rather than a defect in the transport, so it would have been
   * reported to the student as a missing video.
   */
  it('forwards method, body and headers, not just the url', async () => {
    const transport = vi.fn(async () => new Response('{}'));
    listLanguagesMock.mockImplementation(async (_id: string, cfg: Record<string, unknown>) => {
      const via = cfg.playerFetch as (p: unknown) => Promise<Response>;
      await via({
      ...
      return [{ languageCode: 'fr', languageName: 'French', isAutoGenerated: true }];
    });
    fetchTranscriptMock.mockResolvedValue([{ text: 'bonjour' }]);
    ...
    expect(url).toContain('/youtubei/v1/player');
